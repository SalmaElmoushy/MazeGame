...
def part_one():
    ...
    print_with_delay("Which door do you choose? (left, middle, or right): \n", 0.03)
    # ask user if they want a hint
    print_with_delay("Do you want a hint? (yes or no)", 0.03)
    answer = get_valid_input(['yes', 'no'])
    if answer == 'yes':
        print_with_delay("The left door contains a pile of treasure. The right door is an empty room. The middle door contains a monster.", 0.03)
        total_score -= 5
    else:
        print_with_delay("Alright, let's move on.", 0.03)
    print_with_delay("Which door do you choose? \n", 0.03)
    door_choice = get_valid_input("Which door do you choose? (left, middle, or right): ")
    if door_choice == "left":
        print_with_delay("You open the door and see a pile of treasure!\n", 0.03)
        total_score += 10
    elif door_choice == "middle":
        print_with_delay("You open the door and see a monster!\n", 0.03)
        total_score -= 5
    else:
        print_with_delay("You open the door and see an empty room.\n", 0.03)
